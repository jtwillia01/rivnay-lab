...
| Publications | `/publications/` | `_data/publications.yml` (generated) + `_data/publications_manual.yml` |
...
`_data/publications.yml` (generated by `scripts/publications.py`, do not edit):

```yaml
- year: 2024
  authors: Z. Laswick, X. Wu, A. Surendran, J. Rivnay
  ...
  volume: "15"
  pages: "6309"
  doi: 10.1038/s41467-024-50496-6
  openalex: W4400927226
```

`_data/publications_manual.yml`: `exclude:` list of DOIs or OpenAlex IDs to drop, `add:` list of entries in the same shape (used for the pre-2017 list and anything OpenAlex lacks). `pre_northwestern: true` on an entry puts it under the collapsed heading.
...
- The script prints a diff report against `scratch/current-site-publications.txt` (the 69 old entries) so nothing is silently lost; unmatched old entries go into `publications_manual.yml` `add:`.
...
- Publications diff report shows zero unmatched old entries after manual additions.
- 65 posts in `_posts`.
